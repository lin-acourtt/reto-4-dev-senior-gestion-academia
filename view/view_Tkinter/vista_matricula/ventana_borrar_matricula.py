...
class VentanaBorrarMatricula(ctk.CTk):
    def __init__(self, parent=None):
        ...
        self.label_titulo = ctk.CTkLabel(self, text="Borrar matricula",font=("Helvetica", 14, "bold"))
        #self.label_titulo.grid(row = 0, column = 0, columnspan=2)
        self.label_titulo.pack(pady=10)

        # Se obtienen el ID del matricula seleccionado en la tabla
        iid_sel = self.parent.frame_tabla_matriculas.tabla_matriculas.selection()[0]
        self.label_msj = ctk.CTkLabel(self, text=f"¿Acepta borrar el matricula con ID: {iid_sel}?")
    ...
    def confirmar_borrado(self,id_sel):
        ...
        try:
            # Se utiliza el controlador matricula para eliminar un matricula por el ID especificado
            self.parent.controlador_matricula.eliminar_matricula_por_id(id_sel)
            
            # Actualizar tabla en el tree view
            self.parent.frame_tabla_matriculas.tabla_matriculas.delete(id_sel)
            
            # Dar un mensaje de confirmación
            msg_eliminacion_exitosa("Matrícula")
            self.actualizar_estado_ventana_al_cerrar()
        except IntegrityError as e:
            msg_error_integrity("matricula",str(e.msg))
        except Exception as e: 
            msg_error_inesperado("matricula",str(e))
